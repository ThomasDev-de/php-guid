...
class GUID
{
	 ...
	 */
	public static function isValid(string $guid, bool $onlyCaseInsensitive = false): bool
	{
		return !empty(preg_match(self::$pattern . (!$onlyCaseInsensitive ? 'i' : ''), trim($guid)));
	}
	 ...
	 * @noinspection PhpUnused
	 */
	public static function isEmpty(?string $guid = null): bool
	{
		return null != $guid && !empty(preg_match(self::$patternEmpty, trim($guid)));
	}
	 ...
	 * @throws Exception
	 */
	public static function generate(bool $onlyCaseInsensitive = false): string
	{
		...
		return !$onlyCaseInsensitive ? strtolower($guid) : $guid;
	}
